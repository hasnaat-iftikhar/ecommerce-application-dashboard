...
export const BrandActionsCell: React.FC<{ row: Row<BrandType> }> = ({
  ...
  const deleteMutation = useMutation(
    async (id: string) => await axios.delete(`/api/brand?id=${id}`),
    {
      onSuccess: async (data) => {
        await queryClient.invalidateQueries(["fetchingBrands"]);
      ...
      onError: () => {
        toast({
          variant: "destructive",
          title: "Error!",
          description: "Unable to delete the brand",
        });
      },
    }
  );
      ...
      {deleteMutation?.isLoading ? (
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <Link href={ROUTES.EDIT_BRAND(row.original.id)}>
              <DropdownMenuItem>Edit</DropdownMenuItem>
            </Link>
            <DropdownMenuItem onClick={handleDeleteBrand}>
              Delete
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
